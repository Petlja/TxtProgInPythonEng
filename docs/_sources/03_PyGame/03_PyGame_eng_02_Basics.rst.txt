...
These steps at the beginning and at the end of runtime are the same, or very similar, in every program, so we have decided to move these steps to a separate module called *petljapg*. Because of this our module *petljapg* has to be imported as well at the beginning of the programs in our examples. In turn, our programs will become shorter and simpler, because instead of writing statements that perform the actions listed above, we only have one function call from the *petljapg* module at the beginning and one at the end of the program.
...
.. activecode:: pygame__basics_first_example_eng
    :nocodelens:
    :modaloutput: 

    import pygame as pg
    import petljapg
    canvas = petljapg.open_window(400, 400, "Pygame")
...
- using the ``import petljapg`` statement we import module *petljapg* into our program. This statement can be combined with the previous one to get a single statement: ``import pygame as pg, petljapg``, and we will often do this.
- The ``canvas = petljapg.open_window(400, 400, "Pygame")`` statement calls the ``open_window`` function from the ``petljapg`` module, which we imported into the program. This function performs all the necessary preparatory actions that we mentioned earlier. The function parameters are the width, height and title of the window opened by calling this function. The variable *canvas* returned by this function is later used in the program to draw in that window.
...
.. activecode:: pygame__basics_coordinates_eng
   :passivecode: true
...
.. activecode:: pygame__basics_learn_coordinates_eng
   :nocodelens:
   :modaloutput:
   :playtask:
   :includehsrc: src/PyGame/1_Drawing/1_BasicExamples/learn_coordinates_eng.py
...
.. dragndrop:: pygame__basics_quiz_coordinates_circles_eng
    :feedback: Try again!
    :match_1: red|||(30, 40)
...
.. fillintheblank:: pygame__basics_quiz_coordinates_vindow_center_eng
...
.. mchoice:: pygame__basics_quiz_coordinates_dir_eng
   :multiple_answers:
...
.. dragndrop:: pygame__basics_quiz_coordinates_corners_eng
    :feedback: Try again!
...
.. activecode:: colors_eng
   ...
   canvas = petljapg.open_window(400, 400, "Color names")
...
.. activecode:: colors_rgb_eng
   ...
   # start working with the PyGame library
   canvas = petljapg.open_window(400, 400, "RGB Colors")
   # -*- acsection: main -*-
...
.. commented out
    ...
    .. dragndrop:: pygame__basics_quiz_color_names_eng
        :feedback: Try again!
...
.. dragndrop:: pygame__basics_quiz_color_values_eng
    :feedback: Try again!
    :match_1: Black|||(0, 0, 0)
...
.. mchoice:: pygame__basics_quiz_color_gray_eng
   :answer_a: (1, 12, 123)
   :answer_b: (128, 0, 128)
   :answer_c: (0, 0, 128)
...
.. mchoice:: pygame__basics_quiz_color_purple_eng
...
.. mchoice:: pygame__basics_quiz_color_approx_eng
   :answer_a: Bluish
   :answer_b: Reddish
   :answer_c: Yellowish
   :answer_d: Greenish
...
.. dragndrop:: pygame__basics_quiz_colors_and_coordinates_eng
    :feedback: Try again!
